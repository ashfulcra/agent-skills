...
Ties these `fulcra-agent-*` skills together for **unattended** operation. `fulcra-agent-reconcile` heals a
team's index/views, but someone has to run it; this skill **schedules** it, and makes wake-ups
(cron/heartbeat) **resume structured continuity** first. Scheduling is a single, platform-specific action
(not a fold), so this skill is prose + bundled install scripts — no engine logic.
...
the heartbeat: validated inputs, pinned `PATH`/`HOME` (scheduled jobs source no profile), `plutil` lint,
...
platform's own session events. The contract itself (rules 1–4) lives in the `fulcra-agent-continuity`
skill (proposed in PR #129); the adapters below automate it. Each keys everything on a distinct
`fulcra-coord2` marker and coexists with any legacy `fulcra-coord` adapters — installing one never touches
a legacy entry. All installers are idempotent (reinstall replaces, never duplicates) and ship an
`--uninstall` inverse.
...
references:** after upgrading the skill, **RE-RUN YOUR ADAPTER INSTALLER** to regenerate them —
...
(the `fulcra-agent-continuity` skill, proposed in PR #129), and use the platform's scheduled routines to
...
3. **Gate merges** with the `fulcra-agent-review` skill (`review status`), and keep roles fresh with
   `fulcra-agent-roles` (`roles status`), escalating vacancies. (Both proposed in PR #129.)

That's the full `fulcra-agent-*` stack running unattended on top of a `fulcra-agent-teams` space.
