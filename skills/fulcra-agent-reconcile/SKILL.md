...
## What it does
A stdlib-only tool (`coord-engine`) that, for a given team:
...
## Usage
This skill drives the shared **`coord-engine`** tool, so the skill itself stays pure prose + references
(no bundled code). Needs `fulcra-api` authenticated and `coord-engine` installed (see
[Installation](#installation)). See [`references/reconcile-cli.md`](references/reconcile-cli.md).
```bash
